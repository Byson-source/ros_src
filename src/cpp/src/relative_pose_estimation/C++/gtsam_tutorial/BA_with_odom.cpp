...
//   See SFMData.h
  ...
  auto poseNoise = noiseModel::Diagonal::Sigmas(
      (Vector(6) << Vector3::Constant(0.1), Vector3::Constant(0.3))
        ...
        //   fnishedを使うことで一行分の初期化に当たる部分を省略できる
  ...
  // graph
  // カメラのポーズごとのポイントの3D座標を検出している

  std::vector <Symbol> pose_symbol;
  // std::vector <Symbol> points_symbol;

  // GenericValue<Pose3> odometry=
  // Pose3(Rot3::Ypr(0,0,0), Point3(0, 0, 2));
  // // create a measurement for both factors (the same in this case)
  // auto odometryNoise = noiseModel::Diagonal::Sigmas(
  //     Vector3(0.2, 0.2, 0.1));  // 20cm std on x,y, 0.1 rad on theta per 2m

  // Add odometry factors
  // ↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓
  const Pose3 odometry=Pose3(Rot3::Ypr(0,0,0),Point3(2,0,0));
  ...
  for (size_t i = 0; i < poses.size(); ++i) {
    ...
    pose_symbol.push_back(Symbol('x',i));
    if(i<poses.size()-1)
      graph.emplace_shared<BetweenFactor<Pose3>>(Symbol('x', i),Symbol('x', i+1),odometry,odometryNoise);
  ...
  // for(int i{0};i<pose_symbol.size()-1;++i){
  //   graph.emplace_shared<BetweenFactor<Pose3>>(pose_symbol[i],pose_symbol[i+1],odometry,odometryNoise);
  // }
...
//   適当な数値を入れる
  ...
  marginals.marginalCovariance(pose_symbol[0]);
